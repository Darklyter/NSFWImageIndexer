...
def setup_koboldcpp_terminal(model, gpu_summary):
    ...
    if executable_path is None:
        print("Error: KoboldCPP executable not found.")
        print("Please run setup with --update to download it first.")
        return False
    ...
    full_command = f"{executable_path} {kobold_args['model_param']} --mmproj {kobold_args['mmproj']} {flashattention_flag}--usecublas --gpulayers {kobold_args['gpulayers']} --contextsize {kobold_args['contextsize']} --visionmaxres 9999 --chatcompletionsadapter {kobold_args['chatcompletionsadapter']}"
...
class SetupApp:
    ...
    def run_setup(self, update=True):
        ...
        if update:
            ...
            if existing_executable is None:
                QMessageBox.warning(None, "No Executable Found",
                    "KoboldCPP executable not found.\n\n"
                    "Please run 'Install Requirements' first to download it.")
                return None
            gpu_summary["executable_path"] = existing_executable
