This thing is a bit of a mess, organizationally speaking.  It's an LLM powered image indexer that is just a modified version of [ImageIndexer](https://github.com/jabberjabberjabber/ImageIndexer) designed to save tags and descriptions to postgres so that I can pull them into Stash later.  It's not accurate, but it's "accurate enough for government work".  I use the [Qwen3-VL-8B-NSFW-Caption-V4.5.mmproj-f16.gguf](https://huggingface.co/mradermacher/Qwen3-VL-8B-NSFW-Caption-V4.5-GGUF) model since it seems to be the best for NSFW that is publicly available currently, but if you find something better let me know!  The initial setup should allow you to auto-download it, so no worries about that.  
...
- **PostgreSQL Output**: Optionally write all results to a PostgreSQL database with a curated tag vocabulary, gallery/performer tracking, and studio detection
- **Tag Review Tool**: Standalone GUI (`tag_review.py`) to review unmatched keywords, assign them to canonical tags, and manage the tag vocabulary with fuzzy near-miss suggestions
- **Explore Performers**: Standalone GUI (`explore_performers.py`) to browse performers, review their auto-assigned tags, pin or remove tags per-performer, blacklist tags globally, and manually attach additional tags
...
Results can optionally be written to a PostgreSQL database instead of (or in addition to)
JSON sidecar files. Configure the connection in **Settings → Database Connection** and
select your output mode: **JSON sidecar files**, **PostgreSQL database**, or **Both**.

### Setup

Run `python create_database.py` to create the schema on a fresh PostgreSQL instance.
It reads connection details from `settings.json` automatically, or you can pass them
as arguments:

```
python create_database.py --host localhost --port 5432 --user postgres --dbname mydb
```

Requires PostgreSQL 14+ with the `citext` extension available (ships with PostgreSQL).

### What is stored

- Images, galleries, studios, and performers (auto-detected from file paths and zip filenames)
- Canonical keyword tags with a curated alias vocabulary (loaded from a JSON tag file)
- Raw LLM output, unmatched keywords, and per-run processing history
- Per-performer tag assignments with pin/exclude/manual-add controls

### Tag Review Tool

Run `python tag_review.py` to open a GUI for reviewing unmatched keywords. For each
...
Run `python explore_performers.py` to browse all performers in the database. The tool
shows each performer's images, their auto-assigned tags, and a ranked list of all tags
that appear on their images but are not yet directly assigned.
...
| Button | Purpose |
|---|---|
| Assign Performer Tags | Auto-assign tags that appear on ≥ 40 % of a performer's images |
| Backfill All Normalizers | Promote unmatched keywords matching nudity/pubic-hair/labia patterns |
| Backfill From Raw | Apply the same normalizers to all historical raw LLM output |
